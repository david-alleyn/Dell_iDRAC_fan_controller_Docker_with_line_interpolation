...
# Apply the third-party PCIe card cooling setting once at startup. The setting
# doesn't change between iterations, so there's no need to re-issue the ipmitool
# command (and burn an IPMI round-trip) every CHECK_INTERVAL seconds.
...
while true; do
  ...
  then
    ...
    if $IS_CPU2_TEMPERATURE_SENSOR_PRESENT && [ "$CPU2_TEMPERATURE" -gt "$CPU1_TEMPERATURE" ]
    then
      HIGHEST_CPU_TEMPERATURE=$CPU2_TEMPERATURE
    fi
    DECIMAL_CURRENT_FAN_SPEED=$(calculate_interpolated_fan_speed \
